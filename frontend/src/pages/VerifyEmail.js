...
export default function VerifyEmail() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 16px', background: '#f4faf6' }}>
      <div style={{ width: '100%', maxWidth: 420, background: '#ffffff', border: '1.5px solid rgba(22,163,74,0.15)', borderRadius: 20, padding: 'clamp(24px, 5vw, 40px)', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', textAlign: 'center' }}>
        ...
        <p style={{ color: '#16a34a', fontWeight: 700, marginBottom: 28 }}>{user.email}</p>

        <div style={{ marginBottom: 20, textAlign: 'left' }}>
          <label style={{ display: 'block', color: '#374151', fontSize: '0.85rem', marginBottom: 6, fontWeight: 500 }}>Enter 6-digit OTP *</label>
          <input
            value={otp}
        ...
        <button onClick={verifyOTP} disabled={verifying || otp.length !== 6} style={{
          width: '100%',
          background: otp.length === 6 ? '#16a34a' : '#86efac',
        ...
        <button onClick={resendOTP} disabled={resending} style={{
          ...
          cursor: 'pointer', fontWeight: 600, marginBottom: 20
        }}>
          {resending ? 'Sending...' : "Didn't receive OTP? Resend"}
        </button>

        <div style={{ background: '#f4faf6', borderRadius: 10, padding: '12px 16px', border: '1px solid #dcfce7' }}>
