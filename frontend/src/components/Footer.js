        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 28, marginBottom: 32 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <Link to="/" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Home</Link>
              <Link to="/listings" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Browse Waste</Link>
              <Link to="/map" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Waste Map</Link>
              <Link to="/impact" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Impact</Link>
              <Link to="/register" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Join as Collector</Link>
              <Link to="/register" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>Join as Recycler</Link>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {['Plastic', 'Metal', 'Glass', 'Paper', 'Electronic', 'Textile'].map(t => (
                <Link key={t} to={`/listings?wasteType=${t.toLowerCase()}`} style={{ color: '#9ca3af', fontSize: '0.82rem' }}>{t}</Link>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {['🌊 Ocean Cleanup', '🏔️ Landfill Reduction', '🌱 CO₂ Savings', '💰 Collector Revenue'].map(t => (
                <Link key={t} to="/impact" style={{ color: '#9ca3af', fontSize: '0.82rem' }}>{t}</Link>
              ))}
            </div>
          </div>
        </div>
