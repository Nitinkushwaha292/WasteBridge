...
const sendOTPEmail = async (email, name, otp, collectorType) => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: 'Verify your WasteBridge account',
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 480px; margin: 0 auto; background: #ffffff; border-radius: 16px; overflow: hidden; border: 1px solid #e5e7eb;">
        <div style="background: linear-gradient(135deg, #16a34a, #22c55e); padding: 32px 28px; text-align: center;">
          ...
          <p style="color: rgba(255,255,255,0.85); margin: 6px 0 0; font-size: 0.875rem;">Verify your email address</p>
        </div>
        <div style="padding: 32px 28px;">
          ...
          <div style="background: #f4faf6; border: 2px dashed #16a34a; border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 24px;">
            <div style="color: #6b7280; font-size: 0.8rem; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.08em;">Your OTP Code</div>
            <div style="font-size: 2.5rem; font-weight: 800; color: #16a34a; letter-spacing: 0.2em;">${otp}</div>
            <div style="color: #9ca3af; font-size: 0.75rem; margin-top: 8px;">Valid for 10 minutes</div>
          </div>
          <p style="color: #9ca3af; font-size: 0.8rem; margin: 0;">
            If you didn't register on WasteBridge, ignore this email.
          </p>
        </div>
